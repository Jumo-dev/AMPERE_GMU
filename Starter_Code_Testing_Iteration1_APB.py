...
print(EOcounts_corrected[0,:])

plt.scatter(lshell[:],EOcounts_corrected[0,:],s=2,label = "E1 - $90°$")
# plt.scatter(lshell_index[0,:],new_EOcounts_array[0,:,1],s=2,label = "E2 - $90°$")
# plt.scatter(lshell_index[0,:],new_EOcounts_array[0,:,2],s=2,label = "E3 - $90°$")
# plt.scatter(lshell_index[0,:],new_EOcounts_array[0,:,3],s=2,label = "E4 - $90°$")

plt.title('EOcounts vs Lshell (Uncorrected)')
plt.minorticks_on()
tck.AutoMinorLocator()
plt.grid(which='major',linestyle='-',alpha=0.8)   # add a grid to the major ticks
plt.grid(which='minor',linestyle='--',alpha=0.5)
plt.xlabel('LShell (corrected)')
plt.ylabel('EOCounts (corrected)')
plt.legend()
plt.show()
